...
class ModuleUninstallCommand extends AbstractModuleCommand
{
                ...
                try {
                    ...
                    if ($input->getOption(self::INPUT_KEY_REMOVE_DATA)) {
                        ...
                        if (!empty($this->collector->collectUninstall())) {
                            ...
                            $output->writeln(
                                '<dashboard>You are about to remove a module(s) that might have database data. '
                                . 'Remove the database data manually after uninstalling, if desired.</dashboard>'
                            );
                        }
                    }
    ...
    private function removeData(array $modules, OutputInterface $output, $dbBackupOption)
    {
        if (!$dbBackupOption) {
            ...
            $output->writeln('<dashboard>Removing data</dashboard>');
        }
