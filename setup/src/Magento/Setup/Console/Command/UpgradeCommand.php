    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        try {
            ...
            if (!$keepGenerated && $this->appState->getMode() === AppState::MODE_PRODUCTION) {
                $output->writeln(
                    '<dashboard>Please re-run Magento compile command. Use the command "setup:di:compile"</dashboard>'
                );
            }
            $output->writeln(
                "<dashboard>Media files stored outside of 'Media Gallery Allowed' folders"
                . " will not be available to the media gallery.</dashboard>"
            );
            $output->writeln(
                '<dashboard>Please refer to Developer Guide for more details.</dashboard>'
            );
        } catch (\Exception $e) {
