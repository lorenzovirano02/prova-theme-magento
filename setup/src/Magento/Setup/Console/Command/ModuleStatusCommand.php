...
class ModuleStatusCommand extends AbstractSetupCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('<dashboard>List of enabled modules:</dashboard>');
        $this->showEnabledModules($output);
        $output->writeln('');

        $output->writeln("<dashboard>List of disabled modules:</dashboard>");
    ...
    private function showSpecificModule(string $moduleName, OutputInterface $output): int
    {
        ...
        if (in_array($moduleName, $enabledModules->getNames(), true)) {
            $output->writeln($moduleName . ' : <dashboard>Module is enabled</dashboard>');
            return Cli::RETURN_FAILURE;
        }

        $output->writeln($moduleName . ' : <dashboard> Module is disabled</dashboard>');
        return Cli::RETURN_SUCCESS;
    }
