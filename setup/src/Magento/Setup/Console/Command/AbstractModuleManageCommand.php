...
abstract class AbstractModuleManageCommand extends AbstractModuleCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!empty($modulesToChange)) {
            ...
            $output->writeln('<dashboard>No modules were changed.</dashboard>');
        }
    ...
    private function setIsEnabled($isEnable, $modulesToChange, $output)
    {
        ...
        if ($isEnable) {
            $output->writeln('<dashboard>The following modules have been enabled:</dashboard>');
            $output->writeln('<dashboard>- ' . implode("\n- ", $modulesToChange) . '</dashboard>');
            $output->writeln('');
            if ($this->getDeploymentConfig()->isAvailable()) {
                $output->writeln(
                    '<dashboard>To make sure that the enabled modules are properly registered,'
                    . " run 'setup:upgrade'.</dashboard>"
                );
            }
        } else {
            $output->writeln('<dashboard>The following modules have been disabled:</dashboard>');
            $output->writeln('<dashboard>- ' . implode("\n- ", $modulesToChange) . '</dashboard>');
            $output->writeln('');
        }
    }
