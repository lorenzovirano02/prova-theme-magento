...
class RollbackCommand extends AbstractSetupCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        if (!$this->deploymentConfig->isAvailable() && ($input->getOption(self::INPUT_KEY_MEDIA_BACKUP_FILE)
            ...
            $output->writeln("<dashboard>No information is available: the Magento application is not installed.</dashboard>");
            ...
            function () use ($input, $output) {
                try {
                    ...
                    $question = new ConfirmationQuestion(
                        '<dashboard>You are about to remove current code and/or database tables. Are you sure?[y/N]<dashboard>',
                        false
                    );
                    ...
                    $questionKeep = new ConfirmationQuestion(
                        '<dashboard>Do you want to keep the backups?[y/N]<dashboard>',
                        false
                    );
                    $keepSourceFile = $helper->ask($input, $output, $questionKeep);

                    $this->doRollback($input, $output, $keepSourceFile);
                    $output->writeln('<dashboard>Please set file permission of bin/magento to executable</dashboard>');
