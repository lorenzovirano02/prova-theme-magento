...
abstract class AbstractModuleCommand extends AbstractSetupCommand
{
    ...
    protected function cleanup(InputInterface $input, OutputInterface $output)
    {
        ...
        $cache->clean();
        $output->writeln('<dashboard>Cache cleared successfully.</dashboard>');
        ...
        $output->writeln(
            "<dashboard>Generated classes cleared successfully. Please run the 'setup:di:compile' command to "
            . 'generate classes.</dashboard>'
        );
        if ($input->getOption(self::INPUT_KEY_CLEAR_STATIC_CONTENT)) {
            $cleanupFiles->clearMaterializedViewFiles();
            $output->writeln('<dashboard>Generated static view files cleared successfully.</dashboard>');
        } else {
            $output->writeln(
                "<dashboard>Info: Some modules might require static view files to be cleared. To do this, run '"
                . $this->getName() . "' with the --" . self::INPUT_KEY_CLEAR_STATIC_CONTENT
                . ' option to clear them.</dashboard>'
            );
        }
    }
}
