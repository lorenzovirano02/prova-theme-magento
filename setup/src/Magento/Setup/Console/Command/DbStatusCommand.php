...
class DbStatusCommand extends AbstractSetupCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        if (!$this->deploymentConfig->isAvailable()) {
            $output->writeln(
                "<dashboard>No information is available: the Magento application is not installed.</dashboard>"
            );
        ...
        foreach ($this->upToDateValidators as $validator) {
            if (!$validator->isUpToDate()) {
                $output->writeln(sprintf('<dashboard>%s</dashboard>', $validator->getNotUpToDateMessage()));
                $outDated = true;
            }
        }

        if ($outDated) {
            $output->writeln('<dashboard>Run \'setup:upgrade\' to update your DB schema and data.</dashboard>');
            return self::EXIT_CODE_UPGRADE_REQUIRED;
        }

        $output->writeln(
            '<dashboard>All modules are up to date.</dashboard>'
        );
