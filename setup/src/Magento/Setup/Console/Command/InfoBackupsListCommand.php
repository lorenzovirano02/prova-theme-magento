...
class InfoBackupsListCommand extends Command
{
    ...
    {
        $this->setName('dashboard:backups:list')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($this->file->isExists($backupsDir)) {
            ...
            if (empty($tempTable)) {
                $output->writeln('<dashboard>No backup files found.</dashboard>');
                return \Magento\Framework\Console\Cli::RETURN_SUCCESS;
            }
            $output->writeln("<dashboard>Showing backup files in $backupsDir.</dashboard>");
