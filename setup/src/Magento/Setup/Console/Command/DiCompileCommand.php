    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        try {
            ...
            $progressBar->setFormat(
                '<dashboard>%message%</dashboard> %current%/%max% [%bar%] %percent:3s%% %elapsed% %memory:6s%'
            );
            $output->writeln('<dashboard>Compilation was started.</dashboard>');
            $progressBar->start();
            $progressBar->display();
            ...
            $progressBar->finish();
            $output->writeln('');
            $output->writeln('<dashboard>Generated code and dependency injection configuration successfully.</dashboard>');
