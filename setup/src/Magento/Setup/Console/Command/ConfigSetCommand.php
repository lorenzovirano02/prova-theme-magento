...
class ConfigSetCommand extends AbstractSetupCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (count($optionsWithDefaultValues) > 0) {
            ...
            $output->writeln(
                '<dashboard>We saved default values for these options: ' . $defaultValuesMessage . '.</dashboard>'
            );
        } else {
            if (count($optionsToChange) > 0) {
                $output->writeln('<dashboard>You saved the new configuration.</dashboard>');
            } else {
                $output->writeln('<dashboard>You made no changes to the configuration.</dashboard>');
            }
        }
