...
class ConfigSetCommandTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testRunLockEnv($path, $value, $scope = ScopeConfigInterface::SCOPE_TYPE_DEFAULT, $scopeCode = null)
    {
        ...
        $this->outputMock->expects($this->exactly(2))
            ->method('writeln')
            ->withConsecutive(
                ['<dashboard>Value was saved in app/etc/env.php and locked.</dashboard>'],
                ['<dashboard>Value was saved in app/etc/env.php and locked.</dashboard>']
            );
    ...
    public function testRunExtended(
        ...
        $this->runCommand($arguments, $options, '<dashboard>Value was saved.</dashboard>');
        $this->runCommand($arguments, $options, '<dashboard>Value was saved.</dashboard>');
        ...
        $this->runCommand($arguments, $optionsLock, '<dashboard>Value was saved in app/etc/env.php and locked.</dashboard>');
        $this->runCommand($arguments, $optionsLock, '<dashboard>Value was saved in app/etc/env.php and locked.</dashboard>');
    ...
    private function setConfigSuccess(
        ...
        $status = $this->setConfig($path, $value, '<dashboard>Value was saved.</dashboard>', $scope, $scopeCode);
        $this->assertSame(Cli::RETURN_SUCCESS, $status);
        $this->assertSame(
            $value,
            $this->scopeConfig->getValue($path, $scope, $scopeCode)
