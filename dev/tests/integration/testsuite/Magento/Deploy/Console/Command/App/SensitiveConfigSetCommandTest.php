...
class SensitiveConfigSetCommandTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testExecute($scope, $scopeCode, callable $assertCallback)
    {
        ...
        $outputMock->expects($this->at(0))
            ->method('writeln')
            ->with('<dashboard>Configuration value saved in app/etc/env.php</dashboard>');
    ...
    public function testExecuteInteractive($scope, $scopeCode, callable $assertCallback)
    {
        ...
        $outputMock->expects($this->at(0))
            ->method('writeln')
            ->with('<dashboard>Please set configuration values or skip them by pressing [Enter]:</dashboard>');
        $outputMock->expects($this->at(1))
            ->method('writeln')
            ->with('<dashboard>Configuration values saved in app/etc/env.php</dashboard>');
